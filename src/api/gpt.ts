...
export const CallGPT = async (prompt: string) => {
    const completion = await openai.chat.completions.create({
        messages: [
            {
                role: "system",
                content: "You are an expert on pottery. You are a person who loves pottery very much and wants to convey the knowledge of pottery to those who love it well."
            },
            {
                role: "user",
